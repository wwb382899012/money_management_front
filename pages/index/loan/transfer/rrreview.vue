            ...
            <el-form 
              ...
              <el-form-item 
                ...
                <el-input 
                  ...
                  type="textarea" 
                  placeholder="请输入复核意见"></el-input>
              </el-form-item>
              <el-form-item 
                class="form-footer" 
                label-width="0">
                <el-button 
                  v-if="reviewTransfer.transfer_status == '1' || (reviewTransfer.transfer_status == '2' && reviewTransfer.real_pay_type == '2' && reviewTransfer.loan_status == '3')"
                  :disabled="loading" 
                  size="medium" 
                  type="success" 
                  @click="resolveTransfer('reviewTransfer')">{{ reviewTransfer.transfer_status == '2' && reviewTransfer.real_pay_type == '2' && reviewTransfer.loan_status == '3' ? "重试" : "通过" }}</el-button>
                <el-button 
                  v-if="reviewTransfer.transfer_status == '1' || (reviewTransfer.transfer_status == '2' && reviewTransfer.real_pay_type == '2' && reviewTransfer.loan_status == '3')"
                  :disabled="loading" 
                  size="medium" 
  ...
  methods: {
              ...
              if (this.reviewTransfer.real_pay_type == 2) {
                ...
                loanTransferFlowResolve(params)
                  .then(() => {
                    this.loading = false;
                    this.mhideLoading();
                    this.$message.success('通过借款调拨复核成功');
                    this.$router.replace({
                      name: 'loanTransferList'
                    });
                  })
    ...
    stopTransfer(formName) {
      ...
      if (
        ...
        this.mconfirm('确定要拒绝借款调拨吗?')
          .then(() => {
            this.mshowLoading('正在拒绝借款调拨...');
            ...
            loanTransferFlowStop(params)
              .then(() => {
                this.loading = false;
                this.mhideLoading();
                this.$message.success('借款调拨拒绝成功');
                this.$router.replace({
                  name: 'loanTransferList'
                });
              })
