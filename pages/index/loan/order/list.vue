  ...
  <div class="app-page-body">
    ...
    <div class="search-form-box">
      ...
      <el-form 
        ...
        <el-form-item
          label="借款编号">
          <el-input placeholder="请输入借款编号" v-model="searchForm.order_num"></el-input>
        </el-form-item>
        <el-form-item
          label="外部指令号">
    ...
    <el-table 
      ...
      <el-table-column 
        ...
        <template slot-scope="scope">
          <el-button 
            v-if="$store.getters.auth == 'admin' || $store.getters.auth.indexOf('com.jyblife.logic.bg.loan.LoanOrderDetail') != -1"
            size="mini" 
            type="primary" 
            @click="triggerDetail(scope.row.uuid)">详情</el-button>
          <el-button 
            v-if="scope.row.order_status == '0' && ($store.getters.auth == 'admin' || $store.getters.auth.indexOf('com.jyblife.logic.bg.loan.LoanOrderReview') != -1)" 
            size="mini" 
            type="success" 
...
export default {
  ...
  methods: {
    // 表格数据
    getLoanOrderListData(mshowLoading) {
    ...
    handleDownload() {
      ...
      this.mconfirm('确定要导出当前搜索条件的借款指令列表数据吗?')
        .then(() => {
