        ...
        <el-form 
          ...
          <el-form-item 
            ...
            <el-button 
              ...
              size="medium" 
              type="success" 
              @click="resolveAuditRedemption('reviewFinancialPlan')">通过</el-button>
            <el-button 
              v-if="reviewFinancialPlan.pay_status == 2 && reviewFinancialPlan.is_pay_off != 2 && needExpireAudit"
              :disabled="loading" 
              size="medium" 
              type="warning" 
              @click="rejectAuditRedemption('reviewFinancialPlan')">驳回</el-button>
            <tp-historyback 
...
export default {
  ...
  methods: {
    ...
    resolveAuditRedemption() {
      ...
      this.loading = true;
      this.mconfirm('确认审核赎回现金流吗?')
        .then(() => {
          this.mshowLoading('正在审核赎回现金流...');
          let cash_flow = [];
    ...
    // 驳回审核
    rejectAuditRedemption() {
      if (this.loading) return false;
      this.loading = true;
      this.mconfirm('确认驳回赎回现金流吗?')
        .then(() => {
          this.mshowLoading('正在驳回赎回现金流...');
          let cash_flow = [];
