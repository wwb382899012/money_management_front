    ...
    <el-form 
      ...
      <el-form-item 
        ...
        <el-button 
          ...
          size="medium" 
          type="primary" 
          @click="submitForm('newSystem')">保存</el-button>
        <el-button 
          :disabled="loading" 
          size="medium" 
          type="default" 
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.mconfirm('确定要提交吗？')
            .then(() => {
              this.mshowLoading('正在新增业务系统...');

              addSystem(this.newSystem)
                .then(() => {
                  this.loading = false;
                  this.mhideLoading();
                  this.$message.success('新增成功');
                  this.$emit('addSystemSuccess');
                })
                .catch(() => {
                  this.loading = false;
                  this.mhideLoading();
